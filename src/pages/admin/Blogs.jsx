...
import { useBlogs, createBlog, updateBlog, deleteBlog } from "../../services/blogService";
import { useDebounce } from "../../services/productService";
...
export default function Blogs() {
    ...
    const list = useMemo(() => {
        ...
        if (!dq.trim()) return sorted;
        const s = dq.trim().toLowerCase();
        return sorted.filter((b) => String(b.title).toLowerCase().includes(s) || String(b.date).toLowerCase().includes(s));
    }, [data, dq]);
        ...
        <div className="space-y-5">
            <div className="flex items-end justify-between gap-3 flex-wrap">
                <div>
                    ...
                    <div className="mt-2 text-2xl font-black">Blogs</div>
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_220px] items-start">
                <div>
                    <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by title or date..." />
                </div>
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <tr>
                                        <td className="px-5 py-10 text-center text-slate-600" colSpan={3}>
                                            No posts found.
                                        </td>
                                    </tr>
                                ) : (
                                    list.map((b) => (
                                        <tr key={b.id} className="border-b border-slate-200 hover:bg-slate-50/60">
                                            <td className="px-5 py-4">
                                                <div className="flex items-center gap-3 min-w-[360px]">
                                                    <img src={b.image} alt={b.title} className="h-12 w-12 rounded-xl object-cover border border-slate-200" />
                                                    <div>
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex justify-end gap-2">
                                                    <Button variant="outline" onClick={() => onEdit(b)} type="button">
                                                        Edit
                                                    </Button>
                                                    <Button variant="danger" className="bg-red-600 hover:bg-red-700" onClick={() => onDelete(b.id)} type="button">
                                                        Delete
                                                    </Button>
                                                </div>
                                            </td>
